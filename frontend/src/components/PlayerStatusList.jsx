...
export default function PlayerStatusList({ sold = [], remaining = [], currentId = null }) {
  const [filter, setFilter] = useState("available"); // available | sold | all

  const combined = useMemo(() => {
    const list = [
      ...sold.map((p) => ({ ...p, status: "Sold" })),
      ...remaining.map((p) => ({ ...p, status: "Available" })),
    ];

    const filtered = list.filter((p) =>
      filter === "all" ? true : filter === "sold" ? p.status === "Sold" : p.status === "Available"
    );
      ...
      <div className="max-h-[58vh] overflow-y-auto pr-1 space-y-1">
        {combined.map((p) => {
          ...
          return (
            <div
              key={`${p.id}-${p.status}`}
              className={`flex items-start justify-between rounded-lg px-3 py-2 border border-border bg-slate-900/70 ${
        ...
        {combined.length === 0 && <p className="text-xs text-slate-400">No players loaded</p>}
      </div>
    </div>
  );
}
