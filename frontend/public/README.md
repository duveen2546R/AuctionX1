# IPL Auction Game (React + Node + MySQL)
...
- Backend: Node/Express + Socket.IO server, MySQL (fallback in-memory data if DB unavailable).
...
- The game cycles through the player list (from MySQL `players` table). After the final player, the server computes team ratings and broadcasts the winner + scoreboard.
...
- Runtime room state lives in memory for speed; bids, winners, budgets persist to MySQL.
...
## Notes
- Backend automatically falls back to a built-in player list if the database is unreachable, so development works even before MySQL is set up.
